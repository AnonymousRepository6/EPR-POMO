...
from LKH import save_lkh3_result, load_lkh3_result, LKH_solution
...
class TSPTrainer:
    ...
    def _train_one_epoch(self, epoch):
        ...
        lkh_file_path = self.trainer_params['lkh_file_path']
        cuda_device_num = self.trainer_params['cuda_device_num']
        if self.early_stop:
        ...
        for b in range(self.meta_params['B']):
            ...
            for step in range(self.meta_params['k']):
                ...
                if self.meta_params['meta_method'] in ['reptile', 'fomaml']:
                    avg_score, avg_loss = self._train_one_batch(task_model, data, Env(**env_params), lkh_file_path=lkh_file_path, epoch=epoch, cuda_device_num=cuda_device_num, optimizer=optimizer)
                elif self.meta_params['meta_method'] == 'maml':
                    avg_score, avg_loss, fast_weight = self._train_one_batch_maml(fast_weight, data, Env(**env_params), lkh_file_path=lkh_file_path, epoch=epoch, cuda_device_num=cuda_device_num, create_graph=True)
            ...
            for step in range(self.meta_params['L']):
                data = self._get_data(batch_size, task_params)
                if self.meta_params['meta_method'] == 'maml':
                    avg_score, avg_loss, bootstrap_model = self._train_one_batch_maml(bootstrap_model, data, Env(**env_params), lkh_file_path=lkh_file_path, epoch=epoch, cuda_device_num=cuda_device_num, create_graph=False)
                else:
                    avg_score, avg_loss = self._train_one_batch(bootstrap_model, data, Env(**env_params), lkh_file_path=lkh_file_path, epoch=epoch, cuda_device_num=cuda_device_num, optimizer=bootstrap_optimizer)
    ...
    def _train_one_batch(self, task_model, data, env, lkh_file_path, epoch, cuda_device_num, optimizer=None):

        task_model.train()
        batch_size = data.size(0)
        batch = env.load_problems(batch_size, problems=data, aug_factor=1)
        lkh_file_path = os.path.exists(lkh_file_path)
        ...
        # LKH优化损失函数
        if epoch > 0 and (epoch == 1 or epoch % 100 == 0):
            # if lkh_file_exists:
            #     lkh_costs = load_lkh3_result(lkh_file_path, i)
            # else:
            lkh_costs = LKH_solution(batch, device = torch.device(f"cuda:{cuda_device_num}"))  # hgs_costs为[train_batch, 100]并且是负数
            save_lkh3_result(lkh_file_path, epoch, lkh_costs)
            weight = 0.7
            cost_difference = lkh_costs - reward
            cost_difference = weight / (cost_difference + 1e-6)
            mask1 = (advantage > 0) & (cost_difference > 0)
            loss = - (advantage + cost_difference * 2) * log_prob * mask1 + (-advantage * log_prob) * ~mask1
        else:
    ...
    def _train_one_batch_maml(self, fast_weight, data, env, lkh_file_path, epoch, cuda_device_num, optimizer=None, create_graph=True):

        batch_size = data.size(0)
        batch = env.load_problems(batch_size, problems=data, aug_factor=1)
        lkh_file_exists = os.path.exists(lkh_file_path)
        reset_state, _, _ = env.reset()
    ...
    def _update_task_weight(self, tasks, weights, epoch):
        ...
        for i in range(gap.size(0)):
            ...
            if self.meta_params["solver"] == "lkh3_online":
                ...
                gap[i] = torch.tensor(sum(gap_list) / len(gap_list), dtype=torch.float32, device=gap.device)
