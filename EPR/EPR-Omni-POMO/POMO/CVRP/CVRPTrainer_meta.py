...
from HGS import hgs_solution, save_hgs_costs, load_hgs_costs, compute_cost_difference
...
class CVRPTrainer:
        ...
        for b in range(self.meta_params['B']):
            ...
            for step in range(self.meta_params['k']):
                ...
                if self.meta_params['meta_method'] in ['reptile', 'fomaml']:
                    avg_score, avg_loss = self._train_one_batch(task_model, data, Env(**env_params), hgs_file_path=hgs_file_path, epoch=epoch, cuda_device_num=cuda_device_num, optimizer=optimizer)
                elif self.meta_params['meta_method'] == 'maml':
                    avg_score, avg_loss, fast_weight = self._train_one_batch_maml(fast_weight, data, Env(**env_params), hgs_file_path=hgs_file_path, epoch=epoch, cuda_device_num=cuda_device_num, create_graph=True)
            ...
            for step in range(self.meta_params['L']):
                data = self._get_data(batch_size, task_params)
                if self.meta_params['meta_method'] == 'maml':
                    avg_score, avg_loss, bootstrap_model = self._train_one_batch_maml(bootstrap_model, data, Env(**env_params), hgs_file_path=hgs_file_path, epoch=epoch, cuda_device_num=cuda_device_num, create_graph=False)
                else:
                    avg_score, avg_loss = self._train_one_batch(bootstrap_model, data, Env(**env_params), hgs_file_path=hgs_file_path, epoch=epoch, cuda_device_num=cuda_device_num, optimizer=bootstrap_optimizer)
    ...
    def _train_one_batch(self, task_model, data, env, hgs_file_path, epoch, cuda_device_num, optimizer=None):
    ...
    def _train_one_batch_maml(self, fast_weight, data, env, hgs_file_path, epoch, cuda_device_num, optimizer=None, create_graph=True):
        ...
        if epoch > 0 and (epoch == 1 or epoch % 200 == 0):
            ...
            #     hgs_costs = load_hgs_costs(hgs_file_path, epoch)
            hgs_costs = hgs_solution(batch, device = torch.device(f"cuda:{cuda_device_num}"))
            cost_difference, mask = compute_cost_difference(hgs_costs, reward, advantage, weight=1.0)
            loss = - (advantage + cost_difference) * log_prob * mask + (-advantage * log_prob) * ~mask
        else:
            loss = - advantage * log_prob
        # shape: (batch, pomo)
