...
from utils import rollout, check_feasible, Logger, seed_everything, LKH_solution, save_lkh3_result, load_lkh3_result, compute_cost_difference
...
def validate(model, multiple_width, device, mixed=True):
    ...
    if mixed:
        ...
        # validation dataset
        val_100 = TSPDataset('/home/lining/CVRP/EPR-master/TSP/data/tsp_100_val.pkl', num_samples=1000)
        val_100 = TSPDataset('/home/lining/CVRP/EPR-master/TSP/data/tsp_100_val.pkl', num_samples=1000)
        val_100_loader = DataLoader(val_100, batch_size=500)
        val_200 = TSPDataset('/home/lining/CVRP/EPR-master/TSP/data/tsp_200_val.pkl', num_samples=1000)
        val_200_loader = DataLoader(val_200, batch_size=500)
        val_500 = TSPDataset('/home/lining/CVRP/EPR-master/TSP/data/tsp_500_val.pkl', num_samples=100)
...
def train(model, training, T, start_steps, train_steps, mixed, train_batch_size, problem_size, distribution, multiple_width, scale_norm, lr, device, logger, fileLogger, dir_path, log_step, lkh_file_path):
    # Initialize env
    env = TSPEnv(multi_width=multiple_width, device=device)
    ...
    for i in trange(train_steps - start_steps + 1):
        ...
        if i > 0 and (i == 1 or i % 1000 == 0):
            ...
            # else:
            lkh_costs = LKH_solution(batch, device=device)  # hgs_costs为[train_batch, 100]并且是负数
            cost_difference, mask1 = compute_cost_difference(hgs_costs, rewards, advantage, weight=0.7)
...
if __name__ == "__main__":
    with open('/home/lining/CVRP/EPR-master/TSP/config.yml', 'r', encoding='utf-8') as config_file:
    ...
    lkh_file_path = config['lkh_file_path']
    ...
    dir_path = '/home/lining/CVRP/EPR-master/TSP/weights/{}_{}'.format(name, ts_name)
    os.mkdir(dir_path)
    ...
    if(logger_name == 'wandb'):
        logger = wandb.init(project="EPR",
                         name=name + ts_name,
                         config=log_config)
    else:
        logger = None
    # Initialize fileLogger
    filename = 'log/{}_{}_{}'.format(name, ts_name, train_batch_size)
