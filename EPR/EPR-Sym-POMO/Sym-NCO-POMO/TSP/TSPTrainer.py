...
from LKH import save_lkh3_result, LKH_solution, load_lkh3_result
...
class TSPTrainer:
    ...
    def _train_one_epoch(self, epoch):
        ...
        while episode < train_num_episode:
            ...
            avg_score, avg_loss = self._train_one_batch(batch_size=batch_size, lkh_file_path=lkh_file_path, epoch=epoch, episode=episode, cuda_device_num=cuda_device_num)
    ...
    def _train_one_batch(self, batch_size, lkh_file_path, epoch, episode, cuda_device_num):
    ...
    #reward = reward.reshape(self.env_params['sr_size'],batch_size, self.env.pomo_size).permute(1,0,2).reshape(batch_size,self.env.pomo_size*self.env_params['sr_size'])
        ...
        if self.env_params['sr_size']>1:
            ...
            # LKH loss
            advantage_lkh = reward - reward.float().mean(dim=1, keepdims=True)
            # shape: (batch, pomo)
            log_prob_lkh = prob_list.log().sum(dim=2)
            # size = (batch, pomo)
            if episode > 0 and (episode == 24):
                # if hgs_file_exists:
                #     hgs_costs = load_hgs_costs(hgs_file_path, epoch)
                lkh_costs = LKH_solution(batch, device = torch.device(f"cuda:{cuda_device_num}")) 
                save_lkh3_result(lkh_file_path, epoch, episode, lkh_costs)
                weight = 1.0
                cost_difference = lkh_costs - reward
                cost_difference = weight / (cost_difference + 1e-6)
                mask = (advantage_lkh > 0) & (cost_difference > 0)
                loss_lkh = - (advantage_lkh + cost_difference) * log_prob_lkh * mask + (-advantage_lkh * log_prob_lkh) * ~mask
            else:
                loss_lkh = - advantage_lkh * log_prob_lkh
            loss_lkh_mean = loss_lkh.mean()

            #loss_mean = loss_pomo_mean + loss_sr_mean - self.trainer_params['alpha'] * similarity.mean()
            loss_mean = loss_pomo_mean + loss_sr_mean + self.trainer_params['alpha'] * sim_loss + loss_lkh_mean
