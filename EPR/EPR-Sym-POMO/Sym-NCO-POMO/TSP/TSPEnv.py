...
import torch
import pickle
from TSProblemDef import get_random_problems, augment_xy_data_by_8_fold, augment_xy_data_by_N_fold
...
class TSPEnv:
    ...
    def use_saved_problems(self, filename, device):
        ...
        for i in range(len(data)):
            if i==0:
                problem = data[i].view(1, -1, 2)
            else:
                problem = torch.cat((problem,data[i].view(1, -1, 2)),dim=0)
        self.saved_problems = problem.to(device)
        self.saved_index = 0
